/**
 * UI Mock 组件测试 T-5：点 Header 聊天按钮跳转到聊天页面
 *
 * 断言：
 *   1. header-chat-button.tsx 文件存在
 *   2. 登录态：按钮是 Link 链接，指向 /chat
 *   3. 未登录态：点击弹出登录提示 Dialog
...
describe("AI 聊天 T-5：点 Header 聊天按钮跳转到聊天页面", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("5-1. header-chat-button.tsx 文件存在", () => {
    expect(fs.existsSync(COMPONENT_PATH)).toBe(true);
  });

  it("5-2. 登录态：按钮是 Link 链接，指向 /chat", async () => {
  ...
  it("5-3. 未登录态：点击按钮弹出登录提示 Dialog，含达妮娅标题", async () => {
    if (!fs.existsSync(COMPONENT_PATH)) return expect(true).toBe(false);
    vi.doMock("next-auth/react", () => ({
