...
describe("AI 聊天 T-1：未登录用户完全禁用（401，不允许 3 句游客体验）", () => {
  ...
  it("1-3. POST 函数体内：先 const session = await auth() → 然后 if (!session) return Response.json(401 + 请登录文案)", () => {
    if (!fs.existsSync(ROUTE_PATH)) return expect(true).toBe(false);
    const src = fs.readFileSync(ROUTE_PATH, "utf-8");
    // 定位 export async function POST 定义块
    const postStart = src.indexOf("async function POST");
    expect(postStart).toBeGreaterThan(0);
    const postBody = src.slice(postStart, postStart + 2500); // 截函数头 2500 字符足够
    // A: 有 auth() 调用
    expect(postBody).toMatch(/await\s+auth\s*\(\s*\)/);
    // B: if (!session) 分支存在
    expect(postBody).toMatch(/if\s*\(\s*!session\s*\)/);
  ...
  it("1-4. 未登录拦截必须位于「调用 AI / Mock 生成」之前（前置校验，不能先算 token 再拒）", () => {
    if (!fs.existsSync(ROUTE_PATH)) return expect(true).toBe(false);
    const src = fs.readFileSync(ROUTE_PATH, "utf-8");
    // 只在 POST 函数体内搜索（避免函数外部的注释/常量里出现 deepseek/astrbot 等关键词被误判为"调用位置"）
    const postStart = src.indexOf("async function POST");
    expect(postStart).toBeGreaterThan(0);
    const postBody = src.slice(postStart, postStart + 2500); // 和 1-3 一样，截函数头 2500 字符足够
    // 在函数体内定位 auth 检查：if (!session)
    const authCheckIdx = postBody.search(/if\s*\(\s*!session\s*\)/);
    expect(authCheckIdx).toBeGreaterThan(0);
    // 任何可能调用 LLM 的地方：fetch(ASTRBOT / deepseek)、Mock 回复生成函数、new ReadableStream、取 PRESET_REPLIES 生成回复
    // 注意：关键词更严格，避免误命中注释里的"DeepSeek V4 Flash"等文字
    const llmOrMockIdx = postBody.search(
      /(fetch\s*\(\s*[`'"][^`'"]*(?:ASTRBOT|api\/v1\/chat|deepseek|openai)|ASTRBOT_API|deepseek(?:[_A-Za-z]*API|[_A-Za-z]*Client)|mockReply|mock\s*sse|MockSSE|MOCK_REPLY|new\s+ReadableStream|PRESET_REPLIES\s*\[|streamText\s*\()/i,
    );
    if (llmOrMockIdx === -1) return; // 可能实现名字不同，跳过；只要有未登录分支就先 OK
